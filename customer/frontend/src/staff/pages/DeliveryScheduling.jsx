...
export default function DeliveryScheduling() {
  ...
  const fetchEligibleOrders = async () => {
    try {
      const res = await axios.get('/api/pos/deliverable-orders');
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ display: 'flex', gap: 10, marginTop: 16 }}>
              <button
                type="submit"
                className="btn btn-primary"
                disabled={loading || !form.order_id || eligibleOrders.length === 0}
              >
              ...
              {deliveries.map((d) => (
                <tr key={d.id}>
                  ...
                  <td style={{ fontSize: 12 }}>
                    {d.scheduled_date
                      ? new Date(d.scheduled_date).toLocaleString('en-PH')
                      : '—'}
                  </td>
