 ...
 *
 * Customer side:
 * - customer can only request consultation or site measurement
 * - request starts as pending
 * - customer can only view/cancel their own requests
 */
...
const buildNotesBlock = ({
  ...
  if (project_description) lines.push(`Project Description: ${project_description}`);
...
router.post("/", authenticate, requireCustomer, async (req, res) => {
  ...
  if (!ALLOWED_PURPOSES.has(purpose)) {
    return res.status(400).json({
      message: "Only Consultation and Site Measurement can be requested online.",
    });
  }
  ...
  try {
    ...
    if (existing.length > 0) {
      return res.status(409).json({
        message: "You already have an active appointment request for that schedule.",
      });
    }
