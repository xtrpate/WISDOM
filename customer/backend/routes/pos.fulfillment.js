...
const APPOINTMENT_PURPOSES = ["consultation", "site_measurement", "installation"];
 ...
 * List ng deliveries with joined order/customer/driver info
 */
router.get("/deliveries", posAccess, async (req, res) => {
  try {
    const [rows] = await db.query(`
      SELECT
        d.id,
...
router.post("/deliveries", posAccess, async (req, res) => {
...
router.patch("/deliveries/:id/status", posAccess, async (req, res) => {
  const { id } = req.params;
 ...
 * List ng appointments with joined order/customer/staff info
 */
router.get("/appointments", posAccess, async (req, res) => {
  try {
    const [rows] = await db.query(`
      SELECT
        a.id,
...
/**
 * POST /api/pos/appointments
 * Manual appointment creation from staff side
 */
router.post("/appointments", posAccess, async (req, res) => {
  try {
    const orderId = toNullableInt(req.body.order_id);
    const customerId = toNullableInt(req.body.customer_id);
    const purpose = normalizeText(req.body.purpose).toLowerCase() || "installation";
 ...
 * Update appointment assignment / status / notes / schedule
 */
router.patch("/appointments/:id", posAccess, async (req, res) => {
  try {
    const { id } = req.params;
 ...
 */
router.patch("/appointments/:id/status", posAccess, async (req, res) => {
  req.url = `/appointments/${req.params.id}`;
