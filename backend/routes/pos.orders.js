...
router.post('/', authenticate, requireStaffOrAdmin, async (req, res) => {
  ...
  if (payment_method === 'cash') {
    ...
    if (Number.isNaN(normalizedCash)) {
      return res.status(400).json({ message: 'Cash received is required for cash payments' });
    }
  }
  ...
  try {
    ...
    const normalizedCashReceived =
      payment_method === 'cash' && cash_received !== null && cash_received !== undefined
        ? parseFloat(cash_received)
        : null;

    const normalizedChange =
      payment_method === 'cash' && change !== null && change !== undefined
        ? parseFloat(change)
        : null;
