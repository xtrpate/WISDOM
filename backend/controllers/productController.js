...
exports.getAll = async (req, res) => {
  try {
    const { search, type, status, category_id, featured, page = 1, limit = 20 } = req.query;
    ...
    if (status)      { where.push('p.stock_status = ?'); params.push(status); }
    if (category_id) { where.push('p.category_id = ?'); params.push(category_id); }
    if (featured)    { where.push('p.is_featured = ?'); params.push(featured === 'true' ? 1 : 0); }
...
exports.getOne = async (req, res) => {
  try {
    ...
    const [variations] = await pool.query(
      'SELECT * FROM product_variations WHERE product_id = ?', [req.params.id]
    );
...
exports.create = async (req, res) => {
  ...
  try {
    ...
    const {
      barcode, name, description, category_id, type = 'standard',
      online_price, walkin_price, production_cost, stock, reorder_point,
      is_featured = false, variations = '[]', bill_of_materials = '[]',
    } = req.body;
    ...
    const [result] = await conn.query(
      `INSERT INTO products
         (barcode, name, description, category_id, type, image_url, is_featured,
          online_price, walkin_price, production_cost, stock, reorder_point)
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`,
      [barcode, name, description, category_id, type, image_url,
       is_featured ? 1 : 0, online_price, walkin_price, production_cost,
       stock, reorder_point]
    );
    ...
    for (const v of parsedVars) {
      await conn.query(
        `INSERT INTO product_variations
         ...
         VALUES (?,?,?,?,?,?,?)`,
        [productId, v.type, v.value, v.name, v.unit_cost, v.selling_price, v.stock || 0]
      );
    }
...
exports.update = async (req, res) => {
  ...
  try {
    ...
    if (req.body.variations) {
      await conn.query('DELETE FROM product_variations WHERE product_id = ?', [req.params.id]);
        ...
        await conn.query(
          `INSERT INTO product_variations
             (product_id, variation_type, variation_value, variation_name,
              unit_cost, selling_price, stock)
           VALUES (?,?,?,?,?,?,?)`,
          [req.params.id, v.type, v.value, v.name, v.unit_cost, v.selling_price, v.stock || 0]
        );
      }
    }
...
exports.remove = async (req, res) => {
  try {
    const [[p]] = await pool.query('SELECT id, name FROM products WHERE id = ?', [req.params.id]);
...
exports.toggleFeatured = async (req, res) => {
  try {
    await pool.query(
      'UPDATE products SET is_featured = NOT is_featured WHERE id = ?', [req.params.id]
    );
    const [[{ is_featured }]] = await pool.query(
      'SELECT is_featured FROM products WHERE id = ?', [req.params.id]
    );
