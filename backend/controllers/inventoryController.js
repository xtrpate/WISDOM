  ...
  try {
    ...
    if ([qty, reorderPoint, unitCost].some(v => Number.isNaN(v) || v < 0)) {
      return res.status(400).json({
        message: 'Quantity, reorder point, and unit cost must be valid non-negative numbers.',
      });
    }
...
exports.createStockMovement = async (req, res) => {
  ...
  try {
    ...
    if (material_id && product_id) {
      await conn.rollback();
      return res.status(400).json({
        message: 'Only one target is allowed per movement: raw material OR build material.',
      });
    }
    ...
    const delta = POSITIVE_MOVEMENT_TYPES.has(type) ? movementQty : -movementQty;
    ...
    if (type === 'in') {
      ...
      await conn.commit();
      return res.status(201).json({
        message: 'Product stock added and raw materials were deducted automatically.',
        id: r.insertId,
      });
    }
