...
export default function BlueprintsPage() {
  ...
  const confirmArchive = async () => {
    ...
    try {
      ...
      toast.error(err?.response?.data?.message || 'Failed to archive blueprint.');
    } finally {
      setArchivingId(null);
    }
  };
  ...
  const handleRestore = async (id) => {
    try {
      ...
      toast.error(err?.response?.data?.message || 'Failed to restore blueprint.');
    } finally {
      setRestoringId(null);
    }
  };
  ...
  const confirmPermanentDelete = async () => {
    ...
    try {
      ...
      toast.error(
        err?.response?.data?.message || 'Failed to permanently delete blueprint.'
      );
    } finally {
      setDeletingId(null);
    }
  ...
  const handleImport = async (e) => {
    ...
    try {
      ...
      toast.error(err?.response?.data?.message || 'Failed to import blueprint file.');
    } finally {
      setImporting(false);
    }
  };
  ...
  const handleCreateBlueprint = async (e) => {
    ...
    try {
      ...
      const newId = res?.data?.id || res?.data?.blueprint?.id || res?.data?.data?.id;
      ...
      toast.error(err?.response?.data?.message || err?.message || 'Failed to create blueprint.');
    } finally {
      setCreating(false);
    }
  };
      ...
      {items.length === 0 ? (
        ...
        >
          {items.map((bp) => {
            ...
            const isImported = String(bp.source || '').toLowerCase() === 'imported';
            const displayDate =
              tab === 'archive'
            ...
            return (
              ...
              >
                ...
                <div style={{ padding: 16 }}>
                  ...
                  {isImported && (
                    <p style={{ fontSize: 11, color: '#64748b', margin: '0 0 4px' }}>
                      Imported reference{bp.file_type ? ` · ${String(bp.file_type).toUpperCase()}` : ''}
                    </p>
                  )}
                  ...
                  {tab === 'archive' && bp.archive_days_left != null && (
                    <p
                      style={{
                        ...
                        margin: '0 0 12px',
                        fontWeight: 700,
                        color: Number(bp.archive_days_left) <= 5 ? '#dc2626' : '#f59e0b',
                      }}
                    >
                  ...
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    {tab !== 'archive' ? (
                      <>
                        <button
                          onClick={() => navigate(`/blueprints/${bp.id}/design`)}
                          style={btnEdit}
                          disabled={isBusy}
                        >
                        ...
                        {isImported && !!bp.file_url && (
                          <button
                            ...
                            style={{ ...btnEdit, background: '#eef2ff', color: '#4338ca' }}
                            disabled={isBusy}
                          >
                            📄 Open File
                          </button>
                        ...
                        <button
                          onClick={() => navigate(`/blueprints/${bp.id}/estimation`)}
                          style={{ ...btnEdit, background: '#f3e8ff', color: '#6b21a8' }}
                          disabled={isBusy}
                        >
                          💰 Estimate
                        </button>
        ...
        <div style={overlay}>
          <div style={modalBox}>
            ...
            <p style={{ fontSize: 12, color: '#64748b', marginBottom: 16, lineHeight: 1.6 }}>
              ...
              <br />
              Imported files can be opened in the design tool as a traceable background/reference.
            </p>
            ...
            <form onSubmit={handleImport}>
              ...
              <div style={{ marginBottom: 12 }}>
                <label style={labelSm}>Blueprint File (PDF / PNG / JPG / JPEG / SVG) *</label>
                <input
                  type="file"
                  required
              ...
              {!!importForm.file && (
                ...
                >
                  <div><strong>Selected:</strong> {importForm.file.name}</div>
                  <div>
                    <strong>Size:</strong>{' '}
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
                <button
                  ...
                  style={{
                    ...
                    cursor: archivingId === archiveTarget.id ? 'not-allowed' : 'pointer',
                  }}
                >
                  Cancel
                </button>
                ...
                >
                  {archivingId === archiveTarget.id ? 'Archiving...' : 'Yes, Move to Archive'}
                </button>
              </div>
            </div>
          </div>
      ...
      {deleteConfirmModal && deleteTarget && (
        <div style={overlay}>
          <div style={{ ...modalBox, width: 430 }}>
            ...
            >
              This action cannot be undone and may also remove related estimation data.
            </p>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 10 }}>
              ...
              >
                {deletingId === deleteTarget.id ? 'Deleting...' : 'Yes, Delete Permanently'}
              </button>
            </div>
          </div>
        </div>
