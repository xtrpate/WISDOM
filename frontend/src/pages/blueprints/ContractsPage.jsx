...
export default function ContractsPage() {
  ...
  const printContract = (c) => {
    try {
      ...
      doc.text(
        `Date Issued:   ${new Date(c.created_at).toLocaleDateString('en-PH')}`,
        120,
        44
      );
      doc.text(
        `Total Amount:  ₱ ${Number(c.total_amount || 0).toLocaleString('en-PH', {
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 24 }}>
        ...
        <SummaryCard label="This Month" value={contractsThisMonth} color="#10b981" icon="📅" />
      </div>
              ...
              contracts.map((c) => (
                <tr key={c.id} style={{ borderBottom: '1px solid #f1f5f9' }}>
                  ...
                  <td style={{ ...td, fontWeight: 600 }}>
                    {c.total_amount
                      ? `₱ ${Number(c.total_amount).toLocaleString('en-PH', {
                          minimumFractionDigits: 2,
                        })}`
                      : '—'}
                  </td>
                  ...
                  <td style={td}>
                    {c.blueprint_id ? (
                      <button
                        onClick={() => navigate(`/blueprints/${c.blueprint_id}/design`)}
                        style={linkBtn}
                      >
                  ...
                  <td style={td}>
                    <div style={{ display: 'flex', gap: 6 }}>
                      ...
                      <button onClick={() => navigate(`/orders/${c.order_id}`)} style={btnView}>
                        View Order
                      </button>
                    </div>
                  </td>
        ...
        <div style={overlay}>
          <div style={{ ...modalBox, width: 640 }}>
            ...
            <p style={{ fontSize: 12, color: '#64748b', margin: '0 0 20px' }}>
              Select the confirmed order, link a blueprint (optional), and customize the contract
