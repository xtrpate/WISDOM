...
const parseNotes = (notes) => {
  ...
  lines.forEach((line) => {
    if (line.startsWith("Project Description:")) {
      details.projectDescription = line.replace("Project Description:", "").trim();
...
export default function AppointmentScheduling() {
  ...
  const handleAction = async (appointmentId, payload, okMessage) => {
    ...
    try {
      const res = await axios.patch(`/api/pos/appointments/${appointmentId}`, payload);
      ...
      >
        <div>
          ...
          <p style={{ margin: "6px 0 0", color: "#666", fontSize: 14 }}>
            Professional flow: <strong>Handled By</strong> is the staff who processed the
            request, while <strong>Assigned Provider</strong> is the actual staff who will
      ...
      {showForm && (
        <div className="card" style={{ marginBottom: 20 }}>
          ...
          <form onSubmit={handleManualSubmit}>
            ...
            >
              <strong>Note:</strong> Manual appointments are recorded as confirmed. The
              logged-in staff becomes the <strong>Handled By</strong> user. Provider
              assignment is separate and optional.
            </div>
              ...
              {pendingRequests.map((a) => {
                ...
                return (
                  <tr key={a.id}>
                    ...
                    <td style={{ fontSize: 12 }}>{formatDateTime(a.scheduled_date)}</td>
            ...
            <tbody>
              {managedAppointments.map((a) => (
                <tr key={a.id}>
                  ...
                  <td>
                    {a.handled_by_name || <span style={{ color: "#aaa" }}>—</span>}
                  </td>

                  <td>
                    {a.provider_name || <span style={{ color: "#aaa" }}>Not assigned</span>}
                  </td>
