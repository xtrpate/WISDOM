...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // 1. Direct API call (bypasses the Customer context to guarantee we get the raw token)
      ...
      // 2. Save EVERYTHING exactly where Admin, Staff, and Customer contexts expect it
      localStorage.setItem("wisdom_token", token);
      localStorage.setItem("wisdom_user", JSON.stringify(user));
      localStorage.setItem("token", token); // For customer app
      localStorage.setItem("user", JSON.stringify(user)); // For customer app

      // 3. Sync the Admin store just in case
      useAuthStore.setState({ user: user, token: token });

      // 4. Role-Based Access Control (RBAC) Routing
      // We use window.location.href here to force a fresh page load.
      // This guarantees the Admin/Staff portals boot up cleanly and read the new token!
      if (user.role === "admin") {
