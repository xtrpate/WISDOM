...
export default function FaqsPage() {
  ...
  const openEdit = (faq) => {
    setForm({ question: faq.question, answer: faq.answer, sort_order: faq.sort_order, is_visible: !!faq.is_visible });
    setTarget(faq);
    setModal('edit');
  };
  ...
  const toggleVisibility = async (faq) => {
    ...
    toast.success(faq.is_visible ? 'FAQ hidden from website.' : 'FAQ shown on website.');
    load();
  };
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: 12, marginBottom: 20 }}>
        {[
          ...
          <div key={chip.label} style={{ background: '#fff', borderRadius: 10, padding: '12px 18px', borderLeft: `4px solid ${chip.color}`, boxShadow: '0 1px 6px rgba(0,0,0,.08)', minWidth: 120 }}>
            <p style={{ fontSize: 11, color: '#64748b', margin: 0, textTransform: 'uppercase' }}>{chip.label}</p>
            <p style={{ fontSize: 22, fontWeight: 700, color: '#1e2a38', margin: '4px 0 0' }}>{chip.value}</p>
          </div>
        ))}
      </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
          {faqs.map((faq, i) => (
            <div key={faq.id} style={{
              ...card,
              padding: '18px 20px',
              border: faq.is_visible ? '1px solid #e2e8f0' : '1px dashed #d1d5db',
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', gap: 16 }}>
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                    <span style={{ fontSize: 12, fontWeight: 700, color: '#94a3b8', width: 24 }}>#{faq.sort_order}</span>
                    <span style={{ fontSize: 14, fontWeight: 700, color: '#1e2a38' }}>{faq.question}</span>
                    {!faq.is_visible && (
                      <span style={{ fontSize: 10, background: '#f1f5f9', color: '#64748b', padding: '2px 8px', borderRadius: 8, fontWeight: 600 }}>
                        Hidden
                      </span>
                    )}
                  </div>
                  <p style={{ fontSize: 13, color: '#64748b', margin: 0, lineHeight: 1.6, paddingLeft: 32 }}>
                    {faq.answer}
                  </p>
                </div>
                ...
                <div style={{ display: 'flex', gap: 6, flexShrink: 0 }}>
                  <button
                    ...
                    title={faq.is_visible ? 'Hide from website' : 'Show on website'}
                    style={{ ...btnIcon, background: faq.is_visible ? '#d1fae5' : '#f1f5f9', color: faq.is_visible ? '#065f46' : '#94a3b8' }}
                  >
                    {faq.is_visible ? '👁 Visible' : '🙈 Hidden'}
                  </button>
                  <button onClick={() => openEdit(faq)}       style={{ ...btnIcon, background: '#e0f2fe', color: '#0369a1' }}>✏️ Edit</button>
                  <button onClick={() => handleDelete(faq.id, faq.question)} style={{ ...btnIcon, background: '#fee2e2', color: '#dc2626' }}>🗑</button>
                </div>
              </div>
            </div>
          ))}
            ...
            <form onSubmit={handleSave}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginBottom: 14 }}>
                <div>
                  ...
                  <input
                    type="number" min="1" value={form.sort_order}
                    onChange={e => setF('sort_order', parseInt(e.target.value) || 1)}
                    style={inputFull}
                  />
                  <p style={{ fontSize: 11, color: '#94a3b8', margin: '4px 0 0' }}>Lower number = shown first</p>
                </div>
                <div>
                  ...
                  <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', marginTop: 8 }}>
                    ...
                    <span style={{ fontSize: 13, color: form.is_visible ? '#065f46' : '#64748b', fontWeight: 500 }}>
...
const card      = { background: '#fff', borderRadius: 12, boxShadow: '0 1px 6px rgba(0,0,0,.08)' };
...
const modalBox  = { background: '#fff', borderRadius: 12, padding: 28, width: 560, maxHeight: '88vh', overflowY: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,.3)' };
...
const btnIcon   = { padding: '5px 12px', border: 'none', borderRadius: 6, cursor: 'pointer', fontSize: 12, fontWeight: 600 };
