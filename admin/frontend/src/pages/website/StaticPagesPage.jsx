...
export default function StaticPagesPage() {
  ...
  const [form,    setForm]    = useState({ title: '', content: '', is_visible: true });
  ...
  const load = async () => {
    ...
    try {
      ...
      if (first) setForm({ title: first.title, content: first.content, is_visible: !!first.is_visible });
    } finally {
      setLoading(false);
    }
  };
  ...
  const switchTab = (slug) => {
    ...
    setActive(slug);
    const p = pages[slug];
    if (p) setForm({ title: p.title, content: p.content, is_visible: !!p.is_visible });
    setDirty(false);
    setShowPrev(false);
  };
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button
            ...
            style={{ ...btnGhost, background: showPrev ? '#dbeafe' : '#f1f5f9', color: showPrev ? '#1e40af' : '#374151' }}
          >
      ...
      {dirty && (
        <div style={{ background: '#fef9c3', border: '1px solid #fde68a', borderRadius: 8, padding: '10px 16px', marginBottom: 16, fontSize: 13, color: '#92400e' }}>
          ⚠️ You have unsaved changes on the <strong>{meta?.label}</strong> page.
        </div>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '220px 1fr', gap: 20 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {Object.entries(PAGE_META).map(([slug, m]) => {
            ...
            return (
              <button key={slug} onClick={() => switchTab(slug)} style={{
                padding: '14px 16px', border: 'none', borderRadius: 10, cursor: 'pointer',
                textAlign: 'left', background: isActive ? '#1e40af' : '#fff',
        ...
        <div style={card}>
          ...
          <div style={{ padding: '16px 24px', borderBottom: '1px solid #f1f5f9', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <h3 style={{ margin: 0, fontSize: 15, fontWeight: 700, color: '#1e2a38' }}>
                {meta?.icon} {meta?.label}
              </h3>
              <p style={{ margin: '4px 0 0', fontSize: 12, color: '#94a3b8' }}>{meta?.preview}</p>
            </div>
            {/* Visibility toggle */}
          ...
          <div style={{ padding: 24 }}>
            {!showPrev ? (
                ...
                <div>
                  <label style={labelSm}>
                    ...
                    <span style={{ fontWeight: 400, color: '#94a3b8', marginLeft: 6, fontSize: 11 }}>
                      Supports plain text. Use double line breaks for paragraphs.
                    </span>
                  </label>
                  <textarea
                    value={form.content || ''}
                    ...
                    rows={18}
                    style={{ ...inputFull, resize: 'vertical', lineHeight: 1.7, fontFamily: 'inherit' }}
              ...
              /* ── Preview mode ─────────────────────────────────── */
              <div style={{ background: '#f8fafc', borderRadius: 10, padding: 24, minHeight: 400 }}>
                <div style={{ marginBottom: 6, fontSize: 11, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: 1 }}>
                  Preview — {meta?.label} Page
                </div>
                <h2 style={{ fontSize: 22, fontWeight: 700, color: '#1e2a38', margin: '0 0 16px' }}>
                  {form.title || meta?.label}
                </h2>
                <div style={{ fontSize: 14, color: '#374151', lineHeight: 1.8, whiteSpace: 'pre-wrap' }}>
                  {form.content || <span style={{ color: '#94a3b8', fontStyle: 'italic' }}>No content yet.</span>}
                </div>
                {!form.is_visible && (
                  <div style={{ marginTop: 20, padding: '10px 14px', background: '#fee2e2', borderRadius: 8, fontSize: 12, color: '#991b1b' }}>
                    ⚠️ This page is currently <strong>hidden</strong> from the website.
                  </div>
                )}
              </div>
            )}
