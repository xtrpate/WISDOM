...
export default function SalesReportPage() {
  ...
  const exportPDF = () => {
    ...
    doc.text(
      `SALES REPORT — ${tabLabel.replace(/[🏪🌐📊]/g, "").trim()}`,
      148,
      28,
      { align: "center" },
    );
    ...
    doc.text(
      `Total Profit: ₱ ${Number(s.total_profit).toLocaleString()}`,
      120,
      44,
    );
    doc.text(
      `Avg Order Value: ₱ ${Number(s.avg_order_value).toFixed(2)}`,
      185,
      44,
    );
    ...
    <div ref={printRef}>
      ...
      {s && (
        ...
        >
          ...
          <KpiCard
            label="Total Revenue"
            value={`₱ ${Number(s.total_revenue).toLocaleString("en-PH")}`}
            color="#10b981"
            icon="💰"
          />
          <KpiCard
            label="Total Profit"
            value={`₱ ${Number(s.total_profit).toLocaleString("en-PH")}`}
            color="#8b5cf6"
            icon="📈"
          />
          <KpiCard
            label="Avg Order Value"
            value={`₱ ${Number(s.avg_order_value).toFixed(2)}`}
            color="#f59e0b"
            icon="🧾"
          />
          {tab === "" && (
                      ...
                      return (
                        ...
                        >
                          ...
                          <td style={{ ...td, fontWeight: 600 }}>
                            ₱{" "}
                            {Number(o.total_amount).toLocaleString("en-PH", {
                              minimumFractionDigits: 2,
                            })}
                          </td>
                          <td
                            style={{ ...td, color: "#065f46", fontWeight: 600 }}
                          >
                            ₱ {Number(o.total_profit || 0).toFixed(2)}
                          </td>
                          <td style={td}>
