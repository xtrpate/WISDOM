...
export default function ProcessOrder() {
  ...
  const [form, setForm] = useState({
    ...
    appointment_purpose: 'site_measurement',
  ...
  const change = form.payment_method === 'cash' ? Math.max(cashReceived - total, 0) : 0;

  const normalizedPhone = String(form.customer_phone || '').replace(/\D/g, '');
  const phoneIsRequired = form.need_delivery || form.need_appointment;
  const phoneIsValid =
    !normalizedPhone || isValidPHPhone(normalizedPhone);
  ...
  const deliveryIsValid =
    !form.need_delivery ||
    (form.delivery_address.trim() && form.delivery_scheduled_date);

  const appointmentIsValid =
    !form.need_appointment ||
    (form.appointment_purpose && form.appointment_scheduled_date);
  ...
  const handleSubmit = async (e) => {
    ...
    if (phoneIsRequired && !normalizedPhone) {
      return setError('Phone number is required for delivery or appointment.');
    }

    if (normalizedPhone && !isValidPHPhone(normalizedPhone)) {
      return setError('Enter a valid 11-digit PH mobile number starting with 09.');
    }
    ...
    if (form.need_delivery) {
      ...
      if (!form.delivery_scheduled_date) {
        return setError('Delivery schedule is required.');
      }
    }

    if (form.need_appointment) {
      if (!form.appointment_purpose) {
        return setError('Appointment purpose is required.');
      }

      if (!form.appointment_scheduled_date) {
        return setError('Confirmed appointment date and time is required.');
      }
    }
      ...
      items: cart,
        delivery: form.need_delivery
          ? {
              ...
              scheduled_date: form.delivery_scheduled_date,
              notes: form.delivery_notes.trim()
            }
          : null,
        appointment: form.need_appointment
          ? {
              purpose: form.appointment_purpose,
              preferred_date: form.appointment_preferred_date || null,
              scheduled_date: form.appointment_scheduled_date,
              notes: form.appointment_notes.trim()
            }
          : null
      };
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '60vh' }}>
        <div className="card" style={{ maxWidth: 520, width: '100%', textAlign: 'center', padding: 40 }}>
          ...
          {success.delivery && (
            <p style={{ color: '#666', marginBottom: 6 }}>
              Delivery Scheduled
              {success.delivery.assigned_driver?.name ? ` • Driver: ${success.delivery.assigned_driver.name}` : ''}
            </p>
          )}
          ...
          {success.appointment && (
            <p style={{ color: '#666', marginBottom: 6 }}>
              ...
              {success.appointment.assigned_to?.name ? ` • Assigned To: ${success.appointment.assigned_to.name}` : ''}
            </p>
          )}

          <div style={{ marginBottom: 24 }}>
            <p style={{ fontSize: 22, fontWeight: 800, color: '#8B4513', marginBottom: 8 }}>
              ₱{parseFloat(success.total).toLocaleString('en-PH', { minimumFractionDigits: 2 })}
            </p>

            {success.cash_received !== null && success.cash_received !== undefined && (
              <>
                <p style={{ color: '#666', marginBottom: 4 }}>
                  Cash Received: <strong>₱{parseFloat(success.cash_received || 0).toLocaleString('en-PH', { minimumFractionDigits: 2 })}</strong>
                </p>
                <p style={{ color: '#666', marginBottom: 0 }}>
                  Change: <strong>₱{parseFloat(success.change || 0).toLocaleString('en-PH', { minimumFractionDigits: 2 })}</strong>
                </p>
              </>
            )}
          </div>
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 380px', gap: 20 }}>
        <div className="card">
          <h3 style={{ marginBottom: 20, fontWeight: 700 }}>Customer, Payment & Scheduling</h3>
          ...
          <form onSubmit={handleSubmit}>
              ...
              <div className="form-field">
                ...
                <input
                  ...
                  onChange={e =>
                    setForm({
                      ...form,
                      customer_phone: e.target.value.replace(/\D/g, '').slice(0, 11)
                    })
                  }
                />
                ...
                <select
                  ...
                  onChange={e =>
                    setForm({
                      ...
                      cash_received: e.target.value === 'cash' ? form.cash_received : ''
                    })
                  }
                  required
                >
            ...
            <div className="card" style={{ marginTop: 20, background: '#faf7f4' }}>
              <h4 style={{ marginBottom: 14, fontWeight: 700 }}>Fulfillment Options</h4>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 14, fontWeight: 600 }}>
                  <input
                    ...
                    onChange={e => setForm({ ...form, need_delivery: e.target.checked })}
                  />
                  Need Delivery
                </label>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                    <div style={{ gridColumn: '1 / -1' }}>
                      <label style={{ display: 'block', marginBottom: 6, fontSize: 13, fontWeight: 600 }}>
                        ...
                        <input
                          ...
                          style={{ width: '100%', padding: '12px 14px', border: '1.5px solid #e0e0e0', borderRadius: 8, boxSizing: 'border-box' }}
                        />
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={e => setForm({ ...form, delivery_scheduled_date: e.target.value })}
                        required={form.need_delivery}
                        style={{ width: '100%', padding: '12px 14px', border: '1.5px solid #e0e0e0', borderRadius: 8, boxSizing: 'border-box' }}
                      />
                    </div>
                ...
                <label style={{ display: 'flex', alignItems: 'center', gap: 10, fontSize: 14, fontWeight: 600, marginTop: 6 }}>
                  <input
                    type="checkbox"
                    checked={form.need_appointment}
                    onChange={e => setForm({ ...form, need_appointment: e.target.checked })}
                  />
                  Need Appointment
                </label>
                ...
                {form.need_appointment && (
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14 }}>
                    <div>
                      ...
                      <select
                        value={form.appointment_purpose}
                        onChange={e => setForm({ ...form, appointment_purpose: e.target.value })}
                        style={{ width: '100%', padding: '12px 14px', border: '1.5px solid #e0e0e0', borderRadius: 8, boxSizing: 'border-box' }}
                      >
                    ...
                    <div>
                      ...
                      <input
                        type="datetime-local"
                        value={form.appointment_preferred_date}
                        onChange={e => setForm({ ...form, appointment_preferred_date: e.target.value })}
                        style={{ width: '100%', padding: '12px 14px', border: '1.5px solid #e0e0e0', borderRadius: 8, boxSizing: 'border-box' }}
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        type="datetime-local"
                        value={form.appointment_scheduled_date}
                        onChange={e => setForm({ ...form, appointment_scheduled_date: e.target.value })}
                        required={form.need_appointment}
                        style={{ width: '100%', padding: '12px 14px', border: '1.5px solid #e0e0e0', borderRadius: 8, boxSizing: 'border-box' }}
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={e => setForm({ ...form, appointment_notes: e.target.value })}
                        style={{ width: '100%', padding: '12px 14px', border: '1.5px solid #e0e0e0', borderRadius: 8, boxSizing: 'border-box' }}
                      />
                    </div>
                  </div>
            ...
            {error && (
              <div style={{ background: '#fce4ec', color: '#c62828', padding: '10px 14px', borderRadius: 8, fontSize: 13, marginTop: 16 }}>
                {error}
              </div>
            )}
        ...
        <div className="card" style={{ height: 'fit-content' }}>
          ...
          <div style={{ maxHeight: 280, overflowY: 'auto' }}>
            {cart.map(item => (
              <div
                key={item.key}
                style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px solid #f0f0f0', fontSize: 13 }}
              >
                <div>
            ...
            {form.payment_method === 'cash' && (
              <>
                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, marginTop: 10 }}>
                  ...
                  <span>₱{cashReceived.toLocaleString('en-PH', { minimumFractionDigits: 2 })}</span>
                </div>

                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, marginTop: 6, color: cashReceived >= total ? '#2e7d32' : '#c62828' }}>
                  <span>{cashReceived >= total ? 'Change' : 'Insufficient Cash'}</span>
                  <span>
                    ₱{Math.abs(cashReceived - total).toLocaleString('en-PH', { minimumFractionDigits: 2 })}
                  </span>
                </div>
              </>
            )}
