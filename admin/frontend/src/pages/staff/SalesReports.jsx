        ...
        <>
          <div className="stat-grid" style={{ marginBottom: 20 }}>
            ...
            <div className="stat-card">
              ...
              <div>
                <div className="stat-value">{money(data.totals.grand_total)}</div>
            ...
            <div className="stat-card">
              ...
              <div>
                <div className="stat-value">{money(data.totals.total_discount)}</div>
            ...
            <div className="stat-card">
              ...
              <div>
                <div className="stat-value">{money(data.totals.estimated_profit)}</div>
              ...
              {data.payment_breakdown.length === 0 ? (
                ...
                <ResponsiveContainer width="100%" height={220}>
                  <PieChart>
                    ...
                    <Tooltip formatter={(value, _name, item) => [`${value} transactions`, humanize(item?.payload?.payment_method)]} />
                  </PieChart>
                </ResponsiveContainer>
              )}
            </div>
                ...
                <table className="data-table">
                  ...
                  <tbody>
                    {data.summary.length === 0 ? (
                      <tr>
                        <td colSpan={3} style={{ textAlign: 'center', color: '#aaa', padding: 24 }}>
          ...
          <div className="card">
            <h3 style={{ marginBottom: 14, fontWeight: 700, fontSize: 15 }}>Transaction History</h3>
              ...
              <table className="data-table">
                ...
                <tbody>
                  {!data.transactions || data.transactions.length === 0 ? (
                    <tr>
                      <td colSpan={14} style={{ textAlign: 'center', color: '#aaa', padding: 30 }}>
                    ...
                    data.transactions.map((t) => (
                      <tr key={t.order_id}>
                        ...
                        <td>
                          <div style={{ fontWeight: 600 }}>{t.customer_name || 'Walk-in Customer'}</div>
                          <div style={{ fontSize: 11, color: '#888' }}>{t.customer_phone || 'No phone'}</div>
                        </td>
                        <td>
                          <span className={`badge ${badgeClass(t.payment_method)}`}>
                            {humanize(t.payment_method)}
                          </span>
                        </td>
                        <td>{money(t.subtotal)}</td>
                        <td style={{ color: '#2e7d32' }}>
                          {parseFloat(t.discount || 0) > 0 ? `-${money(t.discount)}` : money(0)}
                        </td>
                        ...
                        <td>
                          {t.delivery_status ? (
                            <span className={`badge ${badgeClass(t.delivery_status)}`}>
                        ...
                        <td>
                          {t.appointment_status ? (
                            <span className={`badge ${badgeClass(t.appointment_status)}`}>
