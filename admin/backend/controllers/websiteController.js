...
exports.getSettings = async (req, res) => {
  try {
    const [rows] = await pool.query('SELECT setting_key, value, group_name FROM website_settings ORDER BY group_name, setting_key');
...
exports.getPage = async (req, res) => {
  try {
    const [[page]] = await pool.query('SELECT * FROM static_pages WHERE slug = ?', [req.params.slug]);
...
exports.triggerManualBackup = async (req, res) => {
  try {
    ...
    res.json({ message: 'Backup completed successfully.', file: fileName, size_kb: sizeKb });
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
};
