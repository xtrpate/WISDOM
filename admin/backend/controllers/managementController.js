...
exports.getAll = async (req, res) => {
  try {
    ...
    if (type)    { where.push('o.type = ?');       params.push(type); }      // schema: o.type
    if (status)  { where.push('w.status = ?');     params.push(status); }
    if (from && to) { where.push('DATE(w.created_at) BETWEEN ? AND ?'); params.push(from, to); }
    if (search)  {
...
exports.updateStatus = async (req, res) => {
  try {
    ...
    if (status === 'fulfilled') {
      ...
      if (req.file) update.replacement_receipt = `/uploads/warranty/${req.file.filename}`;
    }
    const sets = Object.keys(update).map(k => `${k} = ?`).join(', ');
    await pool.query(`UPDATE warranties SET ${sets} WHERE id = ?`, [...Object.values(update), req.params.id]);
  ...
  try {
    ...
    const [[order]] = await pool.query(
      `SELECT o.customer_id, COALESCE(u.name, o.walkin_customer_name) AS customer_name
       FROM orders o LEFT JOIN users u ON u.id = o.customer_id WHERE o.id = ?`, [order_id]
    );
    ...
    const [r] = await pool.query(
      ...
      `INSERT INTO contracts
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, CURDATE())`,
      [order_id, blueprint_id || null, order.customer_id, order.customer_name, warranty_terms, terms, req.user.id]
    );

    // Update order status
    await pool.query("UPDATE orders SET status = 'confirmed' WHERE id = ? AND status = 'pending'", [order_id]);
